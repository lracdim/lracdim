...
export function initVector() {
  ...
  async function examineApi(url) {
    let audit;
    try {
      audit = await api('/api/vector/audits', { method: 'POST', body: { url } });
    } catch (e) {
  ...
  function renderApiReport(res, history_) {
    ...
    reportEl.innerHTML = `
      <div class="vector-report__head">
        ...
        <div class="actions" style="margin:0"><button type="button" class="text-link" data-vector-reset>New examination ↗</button><a class="text-link" href="${esc(shareUrl)}">Report link ↗</a></div>
      </div>
